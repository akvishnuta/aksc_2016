<?php
   ...
   $s = "datetime_l";
    ...
    if($sort == 'c')
        ...
        db_connect();
        $result = mysql_query("SELECT * FROM register_s order by {$s}");
         if ($s == 'datetime_l')
                {$result = mysql_query("SELECT * FROM register_s order by date1 DESC,time1 DESC");}
        if ($s == 'datetime_e')
            {$result = mysql_query("SELECT * FROM register_s order by date1 ASC,time1 ASC");}
